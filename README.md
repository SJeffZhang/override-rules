...
本 Fork 在上游规则基础上，为“花云 + 滕王阁 + 良心云”组合订阅增加了多个专用节点组，并将最终兜底策略组明确命名为“漏网之鱼”。

#### 专用节点组

| 代理组 | 节点匹配条件 | 当前样本节点数 |
| --- | --- | ---: |
| `高级 花云+YZoo-亚洲AI` | 花云或 YToo 的台湾、新加坡、日本高级节点 | 32 |
| `高级 花云+YZoo-台湾` | 花云或 YToo 的台湾高级节点 | 6 |
| `高级 花云+YZoo-新加坡` | 花云或 YToo 的新加坡高级节点 | 11 |
| `高级 花云+YZoo-日本` | 花云或 YToo 的日本高级节点 | 15 |
| `香港-花云+滕王阁+YToo` | 花云、滕王阁香港节点及 YToo 香港高级节点；排除实验性节点 | 26 |
| `新加坡-花云+滕王阁+YToo` | 花云、滕王阁新加坡节点及 YToo 新加坡高级节点；排除实验性节点 | 23 |
| `日本-花云+滕王阁+YToo` | 花云、滕王阁日本节点及 YToo 日本高级节点；排除实验性节点 | 27 |
| `美国-花云+滕王阁+YToo` | 美国节点，但排除良心云节点 | 动态 |
| `小众地区` | 摩尔多瓦、意大利、匈牙利、西班牙、荷兰、巴西、智利、新西兰、印尼、越南、巴基斯坦、以色列、阿联酋、尼日利亚、南非 | 动态 |

节点数不是硬编码值，会根据每次从 Sub-Store 传入的实际节点动态变化。专用组排列在地区组区域，并会同时出现在顶层“选择代理”和各业务策略组的候选列表中。组类型由原有 `grouptype` 参数控制：
...
YToo 与花云一样会被普通国家/地区解析器按节点名称归入对应地区组。专用高级组只接收名称中含“高级”的 YToo 节点；`YToo 香港 01`、`YToo 新加坡 01`、`YToo 日本 01` 等非高级节点仅进入普通地区组，不会进入“花云+滕王阁”专用组。

`美国-花云+滕王阁+YToo`会保留花云、滕王阁、YToo 等机场的美国节点，但剔除节点名中包含“良心云”的美国节点，适合在美国区服务中避开良心云线路。

`小众地区`固定使用 `select` 手动选择类型，不受 `grouptype` 或 `loadbalance` 参数影响。
...
因此，未被前面任何域名、IP、GeoSite 或 Rule Provider 规则匹配的流量，都会进入“漏网之鱼”组。该组包含“选择代理”，以及“选择代理”中的全部候选项，包括专用组、自动选择、故障转移、地区组、低倍率节点、手动选择和 `DIRECT`。
